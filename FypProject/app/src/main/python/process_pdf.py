...
def extract_image_from_coords(page, coords):
    """
    Extract an image from specific coordinates in a PDF page.
    """
    cropped_image = page.within_bbox((coords["x0"], coords["y0"], coords["x1"], coords["y1"])).to_image()
    output = cropped_image.crop((coords["x0"], coords["y0"], coords["x1"], coords["y1"]))
...
def extract_pdf_to_json_with_checkmarks_and_images(pdf_path):
    """
    Extracts data from the PDF and includes images from specified coordinates in the result.
    """
    data = {"pages": []}
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        # Extract image from specified coordinates
        image_coords = {
            "x0": 296.0,
            "y0": 300.91999999999996,
            "x1": 552.5,
            "y1": 642.92,
        }
        page_data["extracted_image"] = extract_image_from_coords(page, image_coords)

        # (Your existing checkmark and table extraction logic here)
